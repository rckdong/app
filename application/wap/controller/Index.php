...
//    详情页
    public function info()
    {
        ...
        if($info){
            $brand = Db::name("GoodsCate")->where("id",$info['cate_id'])->field("id,cate_title,cate_desc,pid")->find();
            if($brand['pid']!='0'){
                $pid = Db::name("GoodsCate")->where("id",$brand['pid'])->field("id,cate_title,cate_desc,pid")->find();
                $info['pid_cate_title'] = $pid['cate_title'];
            }else{
                $info['pid_cate_title'] = '';
            }
            $info['cate_title'] = $brand['cate_title'];
        }else{
            $info['cate_title'] = '';
        }
    ...
    public function has_sale()
    {
        ...
        if (isset($get['page']) && $get['page'] != 0) {
            ...
            foreach ($list as $key => $val) {
                ...
                $str .= '<div class="img img_box"><img src="' . $this->get_full_url($val["ahead_image"]) . '"><img class="img_icon" src="http://chexinyuan.com/static/images/yishou.png"></div>';
