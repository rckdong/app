...
class FansService
{
    ...
    public static function set(array $user)
    {
        ...
        $appuser = [];
        foreach (['unionid','openid','nickname','sex','country','province','city','language','headimgurl','remark'] as $key){
            isset($user[$key]) && $appuser[$key] = $user[$key];
        }

        DataService::save('AppUsers',$appuser,'openid');
