...
class Index extends BasicAdmin
{
    ...
    {
        $url = url('admin/index/oauth', '', true, true) . '?system_id=' . intval(session('user.id'));
    ...
    public function oauth()
    {
        ...
        if (!isset($fans['openid']) || $fans['openid'] == '') {
            return $this->fetch('index/error', ['title' => '绑定失败，稍后重试']);
        }
        if(!isset($get['system_id']) || $get['system_id'] == ''){
            return $this->fetch('index/error', ['title' => '绑定失败，稍后重试']);
        }
        $savedata['id'] = $get['system_id'];
        $savedata['openid'] = $fans['openid'];
        $r = DataService::save("SystemUser",$savedata);
        if(!$r){
            return $this->fetch('index/error', ['title' => '绑定失败，稍后重试']);
        }
        return $this->fetch('index/error', ['title' => '绑定成功，请关闭PC端的弹框']);
    }
}
